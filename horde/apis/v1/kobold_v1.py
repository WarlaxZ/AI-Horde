...
from flask_restx import Api, Namespace, Resource, abort, fields, reqparse

from ... import cm, invite_only, limiter, maintenance, raid
from ...classes import User, WaitingPrompt, Worker
from ...classes import db as _db
from ...classes import processing_generations, waiting_prompts
from ...logger import logger

api = Namespace("v1", "API Version 1")

response_model_generation = api.model(
    "GenerationKoboldV1",
    {
        "text": fields.String,
...
response_model_wp_status_lite = api.model(
    "RequestStatusCheckKoboldV1",
    {
...
response_model_wp_status_full = api.inherit(
    "RequestStatusKoboldV1",
    response_model_wp_status_lite,
...
def get_error(error, **kwargs):
    ...
    if error == ServerErrors.WRONG_CREDENTIALS:
        logger.warning(
            f'User "{kwargs["username"]}" sent wrong credentials for utilizing instance {kwargs["kai_instance"]}'
        )
        return f'wrong credentials for utilizing instance {kwargs["kai_instance"]}'
    if error == ServerErrors.INVALID_PROCGEN:
        logger.warning(
            f'Server attempted to provide generation for {kwargs["id"]} but it did not exist'
        )
        return f'Processing Generation with ID {kwargs["id"]} does not exist'
    if error == ServerErrors.DUPLICATE_GEN:
        logger.warning(
            f'Server attempted to provide duplicate generation for {kwargs["id"]} '
        )
        return f'Processing Generation with ID {kwargs["id"]} already submitted'
    if error == ServerErrors.TOO_MANY_PROMPTS:
        logger.warning(
            f'User "{kwargs["username"]}" has already requested too many parallel requests ({kwargs["wp_count"]}). Aborting!'
        )
    ...
    if error == ServerErrors.TOO_MANY_STEPS:
        logger.warning(
            f'User "{kwargs["username"]}" sent too many steps ({kwargs["steps"]}). Aborting!'
        )
    ...
    if error == ServerErrors.NOT_ADMIN:
        logger.warning(
            f'Non-admin user "{kwargs["username"]}" tried to use admin endpoint: "{kwargs["endpoint"]}". Aborting!'
        )
        return "You're not an admin. Sod off!"
    if error == ServerErrors.MAINTENANCE_MODE:
        logger.info(
            f'Rejecting endpoint "{kwargs["endpoint"]}" because server in maintenance mode.'
        )
        return "Server has entered maintenance mode. Please try again later."
    if error == ServerErrors.NOT_OWNER:
        logger.warning(
            f'User "{kwargs["username"]}" tried to modify server they do not own: "{kwargs["server_name"]}". Aborting!'
        )
...
class SyncGenerate(Resource):
    ...
    parser.add_argument(
        ...
        required=False,
        default={},
        help="Extra generate params to send to the KoboldAI server",
        location="json",
    )
    parser.add_argument(
        "servers",
        ...
        required=False,
        default=[],
        help="If specified, only the server with this ID will be able to generate this prompt",
        location="json",
    )
    parser.add_argument(
        "softprompts",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="If specified, only servers who can load this this world info will generate this request",
    )

    @api.expect(parser)
    @api.marshal_with(
        response_model_generation, code=200, description="Text Generated", as_list=True
    )
    ...
    def post(self):
        ...
        if maintenance.active:
            return (
                f"{get_error(ServerErrors.MAINTENANCE_MODE, endpoint = 'SyncGenerate')}",
                503,
            )
        if args.api_key:
        ...
        if not user:
            return (
                f"{get_error(ServerErrors.INVALID_API_KEY, subject = 'prompt generation')}",
                401,
            )
        ...
        if wp_count >= user.get_concurrency(
            args["models"], _db.get_available_models(lite_dict=True)
        ):
            return (
                f"{get_error(ServerErrors.TOO_MANY_PROMPTS, username = username, wp_count = wp_count)}",
                503,
            )
        ...
        if not server_found:
            ...
            return (
                "No active server found to fulfill this request. Please Try again later...",
                503,
            )
...
class AsyncStatus(Resource):
    ...
    @logger.catch(reraise=True)
    @api.marshal_with(
        response_model_wp_status_full, code=200, description="Images Generated"
    )
    def get(self, id=""):
...
class AsyncGenerate(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "prompt", type=str, required=True, help="The prompt to generate from"
    )
    parser.add_argument(
        "api_key",
        type=str,
    ...
    def post(self):
        ...
        if maintenance.active:
            return (
                f"{get_error(ServerErrors.MAINTENANCE_MODE, endpoint = 'AsyncGenerate')}",
                503,
            )
        if args.api_key:
            user = _db.find_user_by_api_key(args["api_key"])
...
class PromptPop(Resource):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The server's unique name, to track contributions",
    )
    parser.add_argument(
        "model",
        type=str,
        required=True,
        help="The model currently running on this KoboldAI",
    )
    parser.add_argument(
        "max_length",
        type=int,
        required=False,
        default=512,
        help="The maximum amount of tokens this server can generate",
    )
    parser.add_argument(
        "max_content_length",
        type=int,
    ...
    parser.add_argument(
        ...
        required=False,
        default=[],
        help="The usernames which get priority use on this server",
    )
    parser.add_argument(
        "softprompts",
        type=str,
        ...
        required=False,
        default=[],
        help="The available softprompt files on this cluster for the currently running model",
    )
    ...
    def post(self):
        ...
        if not cm.is_ip_safe(request.remote_addr):
            return (
                f"Due to abuse prevention, we cannot accept workers from your IP address. Please contact us on Discord if you feel this is a mistake.",
                403,
            )
        skipped = {}
        user = _db.find_user_by_api_key(args["api_key"])
        if not user:
            return (
                f"{get_error(ServerErrors.INVALID_API_KEY, subject = 'server promptpop: ' + args['name'])}",
                401,
            )
        ...
        if user != server.user:
            return (
                f"{get_error(ServerErrors.WRONG_CREDENTIALS,kai_instance = args['name'], username = user.get_unique_alias())}",
                401,
            )
        server.check_in(
            args["max_length"],
...
class SubmitGeneration(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument(
        "id", type=str, required=True, help="The processing generation uuid"
    )
    parser.add_argument(
        "api_key", type=str, required=True, help="The server's owner API key"
    )
    parser.add_argument(
        "generation", type=str, required=True, default=[], help="The generated text"
    )
    ...
    def post(self):
        ...
        if not user:
            return (
                f"{get_error(ServerErrors.INVALID_API_KEY, subject = 'server submit: ' + args['name'])}",
                401,
            )
...
class TransferKudos(Resource):
    ...
    parser.add_argument(
        "api_key", type=str, required=True, help="The sending user's API key"
    )
    parser.add_argument(
        "amount",
        type=int,
    ...
    def post(self):
        ...
        if not user:
            return (
                f"{get_error(ServerErrors.INVALID_API_KEY, subject = 'kudos transfer to: ' + args['username'])}",
                401,
            )
        ret = _db.transfer_kudos_from_apikey_to_username(
            args["api_key"], args["username"], args["amount"]
        )
        kudos = ret[0]
        error = ret[1]
        if error != "OK":
...
class AdminMaintenanceMode(Resource):
    ...
    parser.add_argument(
        "active", type=bool, required=True, help="Star or stop maintenance mode"
    )
    ...
    def put(self):
        ...
        if not admin:
            return (
                f"{get_error(ServerErrors.INVALID_API_KEY, subject = 'Admin action: ' + 'AdminMaintenanceMode')}",
                401,
            )
        if not os.getenv("ADMINS") or admin.get_unique_alias() not in json.loads(
            ...
            return (
                f"{get_error(ServerErrors.NOT_ADMIN, username = admin.get_unique_alias(), endpoint = 'AdminMaintenanceMode')}",
                401,
            )
...
class ServerSingle(Resource):
    ...
    parser = reqparse.RequestParser()
    parser.add_argument(
        "api_key", type=str, required=True, help="The Admin or server owner API key"
    )
    parser.add_argument(
        "maintenance",
        type=bool,
        required=False,
        help="Set to true to put this server into maintenance.",
    )
    parser.add_argument(
        "paused", type=bool, required=False, help="Set to true to pause this server."
    )
    ...
    def put(self, server_id=""):
        ...
        if not admin:
            return (
                f"{get_error(ServerErrors.INVALID_API_KEY, subject = 'User action: ' + 'PUT ServerSingle')}",
                401,
            )
        ret_dict = {}
        ...
        if args.maintenance != None:
            if not os.getenv("ADMINS") or admin.get_unique_alias() not in json.loads(
                ...
                if admin != server.user:
                    return (
                        f"{get_error(ServerErrors.NOT_OWNER, username = admin.get_unique_alias(), server_name = server.name)}",
                        401,
                    )
        ...
        if args.paused != None:
            if not os.getenv("ADMINS") or admin.get_unique_alias() not in json.loads(
                ...
                return (
                    f"{get_error(ServerErrors.NOT_ADMIN, username = admin.get_unique_alias(), endpoint = 'AdminModifyServer')}",
                    401,
                )
...
class UserSingle(Resource):
    ...
    def put(self, user_id=""):
        ...
        if not admin:
            return (
                f"{get_error(ServerErrors.INVALID_API_KEY, subject = 'Admin action: ' + 'PUT UserSingle')}",
                401,
            )
        if not os.getenv("ADMINS") or admin.get_unique_alias() not in json.loads(
            os.getenv("ADMINS")
            ...
            return (
                f"{get_error(ServerErrors.NOT_ADMIN, username = admin.get_unique_alias(), endpoint = 'AdminModifyUser')}",
                401,
            )
        ret_dict = {}
        if args.kudos:
