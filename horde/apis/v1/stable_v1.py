...
class SyncGenerate(Resource):
    ...
    parser.add_argument(
        ...
        required=False,
        default={},
        help="Extra generate params to send to the SD server",
    )
    parser.add_argument(
        "servers",
        type=str,
    ...
    def post(self):
        ...
        if (
            args["params"].get("height", 512) % 64
            or args["params"].get("height", 512) <= 0
        ):
            return (f"{get_error(ServerErrors.INVALID_SIZE, username = username)}", 400)
        if (
            args["params"].get("width", 512) % 64
            or args["params"].get("width", 512) <= 0
        ):
            return (f"{get_error(ServerErrors.INVALID_SIZE, username = username)}", 400)
        if args["params"].get("steps", 50) > 100:
            return (
                f"{get_error(ServerErrors.TOO_MANY_STEPS, username = username, steps = args['params']['steps'])}",
                400,
            )
        wp = WaitingPrompt(
            _db,
...
class PromptPop(Resource):
    ...
    parser.add_argument(
        ...
        required=False,
        default=512,
        help="The maximum amount of pixels this server can generate",
    )
    parser.add_argument(
        "priority_usernames",
        type=str,
    ...
    def post(self):
        ...
        if not server:
            if invite_only.active:
                return (
                    f"Horde in worker invite mode only. Please use APIv2 if you have an invite.",
                    401,
                )
            server = Worker(_db)
...
class SubmitGeneration(Resource):
    ...
    parser.add_argument(
        ...
        required=False,
        default=[],
        help="The download location of the image",
    )
    parser.add_argument(
        "seed",
        type=str,
        required=True,
        default=[],
        help="The seed of the generated image",
    )
