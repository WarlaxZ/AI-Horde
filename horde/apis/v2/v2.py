...
from ... import cache, cm, invite_only, limiter, logger, maintenance, raid
from ...classes import (
    News,
    Suspicions,
    Team,
...
class GenerateTemplate(Resource):
    ...
    def validate(self):
        ...
        user_limit = self.user.get_concurrency(
            self.args["models"],
            db.get_available_models(waiting_prompts, lite_dict=True),
        )
...
class SyncGenerate(GenerateTemplate):
    ...
    @api.marshal_with(
        models.response_model_wp_status_full, code=200, description="Images Generated"
    )
    @api.response(400, "Validation Error", models.response_model_error)
    @api.response(401, "Invalid API Key", models.response_model_error)
    @api.response(503, "Maintenance Mode", models.response_model_error)
...
class JobSubmit(Resource):
    ...
    def validate(self):
        ...
        self.kudos = self.procgen.set_generation(
            self.args["generation"], seed=self.args["seed"]
        )
...
class TransferKudos(Resource):
    ...
    @api.expect(parser)
    @api.marshal_with(
        models.response_model_kudos_transfer,
        code=200,
...
class UserSingle(Resource):
    ...
    def put(self, user_id=""):
        ...
        if self.args.contact != None:
            ...
            if ret == "Profanity":
                raise e.Profanity(
                    admin.get_unique_alias(), self.args.contact, "worker contact"
                )
...
class HordeModes(Resource):
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="Initiate a graceful shutdown of the horde in this amount of seconds. Will put horde in maintenance if not already set.",
        location="json",
    )
    parser.add_argument(
        "invite_only",
...
class Teams(Resource):
    ...
    def post(self, team_id=""):
        ...
        if ret == "Already Exists":
            raise e.NameAlreadyExists(
                user.get_unique_alias(), team.name, self.args.name, "team"
            )
