...
from ...vars import (
    ...
    thing_divisor,
    thing_name,
    things_per_sec_suspicion_threshold,
)
from .. import args
...
class WaitingPrompt:
    ...
    def start_generation(self, worker):
        ...
        self.refresh()
        logger.audit(
            f"Procgen with ID {new_gen.id} popped from WP {self.id} by worker {worker.id} ('{worker.name}' / {worker.ipaddr})"
        )
    ...
    def get_status(self, lite=False):
        ...
        avg_things_per_sec = (
            self.db.stats.get_request_avg() / thing_divisor
        ) * active_workers
    ...
    def check_for_stale(self):
        while True:
            try:
                ...
                logger.critical(
                    f"Exception {e} detected. Handing to avoid crashing thread."
                )
                time.sleep(10)
...
class ProcessingGeneration:
    ...
    def set_generation(self, generation, **kwargs):
        ...
        self.things_per_sec = self.owner.db.stats.record_fulfilment(
            things=self.owner.things, starting_time=self.start_time, model=self.model
        )
    ...
    def record(self):
        ...
        if self.fake and self.worker.user == self.owner.user:
            ...
            self.worker.record_contribution(
                raw_things=self.owner.things,
                kudos=self.kudos,
                things_per_sec=self.things_per_sec,
            )
            logger.info(
                f"Fake{cancel_txt} Generation worth {self.kudos} kudos, delivered by worker: {self.worker.name}"
            )
        else:
            self.worker.record_contribution(
                raw_things=self.owner.things,
            ...
            logger.info(
                f"New{cancel_txt} Generation worth {self.kudos} kudos, delivered by worker: {self.worker.name}"
            )
    ...
    def abort(self):
        ...
        logger.info(
            f"Aborted Stale Generation from by worker: {self.worker.name} ({self.worker.id})"
        )

    # Overridable function
    def get_gen_kudos(self):
        return self.owner.db.convert_things_to_kudos(
            self.owner.things, seed=self.seed, model_name=self.model
        )
...
class Worker:
    ...
    def report_suspicion(
        ...
        if reason:
            ...
            logger.warning(
                f"Worker '{self.id}' suspicion increased to {self.suspicious}. Reason: {reason_log}"
            )
    ...
    def can_generate(self, waiting_prompt):
        ...
        if any(
            word.lower() in waiting_prompt.prompt.lower() for word in self.blacklist
        ):
            is_matching = False
            skipped_reason = "blacklist"
        if len(waiting_prompt.models) > 0 and not any(
            model in waiting_prompt.models for model in self.models
        ):
            is_matching = False
    ...
    def record_contribution(self, raw_things, kudos, things_per_sec):
        ...
        if things_per_sec / thing_divisor > things_per_sec_suspicion_threshold:
            self.report_suspicion(
                ...
                formats=[round(things_per_sec / thing_divisor, 2)],
            )
    ...
    def modify_kudos(self, kudos, action="generated"):
        ...
        self.kudos_details[action] = round(
            self.kudos_details.get(action, 0) + abs(kudos), 2
        )
    ...
    def get_details(self, details_privilege=0):
        ...
        ret_dict = {
            ...
            "team": {"id": self.team.id, "name": self.team.name}
            if self.team
            else "None",
        }
    ...
    def deserialize(self, saved_dict, convert_flag=None):
        ...
        self.last_check_in = datetime.strptime(
            saved_dict["last_check_in"], "%Y-%m-%d %H:%M:%S"
        )
        ...
        for suspicion in self.suspicions:
            self.suspicious += 1
            logger.debug(
                f"Suspecting worker {self.name} for {self.suspicious} with reasons {self.suspicions}"
            )
        self.check_for_bad_actor()
...
class User:
    ...
    def create_anon(self):
        ...
        self.contributions = {thing_name: 0, "fulfillments": 0}
        self.usage = {thing_name: 0, "requests": 0}
    ...
    def record_usage(self, raw_things, kudos):
        ...
        self.usage[thing_name] = round(
            self.usage[thing_name]
            + (raw_things * self.usage_multiplier / thing_divisor),
            2,
        )
    ...
    def record_contributions(self, raw_things, kudos):
        ...
        if not self.trusted and not self.is_anon():
            ...
            self.modify_kudos(kudos, "accumulated")
        self.contributions[thing_name] = round(
            self.contributions[thing_name] + raw_things / thing_divisor, 2
        )
    ...
    def check_for_trust(self):
        ...
        """
        if (
            self.evaluating_kudos >= int(os.getenv("KUDOS_TRUST_THRESHOLD"))
            and not self.is_suspicious()
            and not self.is_anon()
        ):
    ...
    def receive_monthly_kudos(self):
        ...
        if self.monthly_kudos["last_received"]:
            has_month_passed = datetime.now() > self.monthly_kudos[
                "last_received"
    ...
    def get_concurrency(self, models_requested=None, models_dict=None):
        ...
        for model_name in models_requested:
            ...
            allowed_concurrency += (
                models_dict.get(model_name, {"count": 0})["count"] * 10
            )
    ...
    def report_suspicion(
        ...
        if reason:
            reason_log = suspicion_logs[reason].format(*formats)
            logger.warning(
                f"User '{self.id}' suspicion increased to {self.suspicious}. Reason: {reason}"
            )
    ...
    def serialize(self):
        ...
        if self.monthly_kudos["last_received"]:
            serialized_monthly_kudos["last_received"] = self.monthly_kudos[
                "last_received"
    ...
    def deserialize(self, saved_dict, convert_flag=None):
        ...
        for suspicion in self.suspicions:
            self.suspicious += 1
            logger.debug(
                f"Suspecting user {self.get_unique_alias()} for {self.suspicious} with reasons {self.suspicions}"
            )
        ...
        if (
            serialized_monthly_kudos
            and serialized_monthly_kudos["last_received"] != None
        ):
            self.monthly_kudos["amount"] = serialized_monthly_kudos["amount"]
            self.monthly_kudos["last_received"] = datetime.strptime(
                serialized_monthly_kudos["last_received"], "%Y-%m-%d %H:%M:%S"
            )
        if self.is_anon():
            self.concurrency = 500
            self.public_workers = True
        self.creation_date = datetime.strptime(
            saved_dict["creation_date"], "%Y-%m-%d %H:%M:%S"
        )
        self.last_active = datetime.strptime(
            saved_dict["last_active"], "%Y-%m-%d %H:%M:%S"
        )
        if convert_flag == "kudos_fix":
            multiplier = 20
            if args.horde == "kobold":
                multiplier = 100
            recalc_kudos = (
                self.contributions["fulfillments"] - self.usage["requests"]
            ) * multiplier
            self.kudos = (
                recalc_kudos
                + self.kudos_details.get("admin", 0)
                + self.kudos_details.get("received", 0)
                - self.kudos_details.get("gifted", 0)
            )
        ...
        if duplicate_user and duplicate_user != self:
            if duplicate_user.get_unique_alias() != self.get_unique_alias():
                logger.error(
                    f"mismatching duplicate IDs found! {self.get_unique_alias()} != {duplicate_user.get_unique_alias()}. Please cleanup manually!"
                )
            else:
                logger.warning(
                    f"found duplicate ID: {[self,duplicate_user,self.get_unique_alias(),self.id,duplicate_user.id,duplicate_user.get_unique_alias()]}"
                )
                duplicate_user.kudos += self.kudos
                if duplicate_user.last_active < self.last_active:
                    logger.warning(
                        f"Merging {self.oauth_id} into {duplicate_user.oauth_id}"
                    )
...
class Team:
    ...
    def get_performance(self):
        ...
        if len(all_performances):
            perf_avg = round(
                sum(all_performances) / len(all_performances) / thing_divisor, 1
            )
    ...
    def get_details(self, details_privilege=0):
        """We display these in the workers list json"""
        worker_list = [
            {"id": worker.id, "name": worker.name, "online": not worker.is_stale()}
            for worker in self.db.find_workers_by_team(self)
        ]
    ...
    def serialize(self):
        ...
        for fulfillment in self.fulfillments.copy():
            json_fulfillment = {
                ...
                "deliver_time": fulfillment["deliver_time"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            }
        ...
        for fulfillment in saved_dict.get("fulfillments", []):
            class_fulfillment = {
                ...
                "start_time": datetime.strptime(
                    fulfillment["start_time"], "%Y-%m-%d %H:%M:%S"
                ),
                "deliver_time": datetime.strptime(
                    fulfillment["deliver_time"], "%Y-%m-%d %H:%M:%S"
                ),
            }
...
class Database:
    def __init__(self, convert_flag=None, interval=60):
        ...
        if convert_flag:
            logger.init_warn(
                f"Convert Flag '{convert_flag}' received.", status="Converting"
            )
        ...
        thread.daemon = True
        thread.start()
        monthly_kudos_thread = threading.Thread(
            target=self.assign_monthly_kudos, args=()
        )
    ...
    def initiate_save(self, seconds=0):
    ...
    def register_new_worker(self, worker):
        ...
        logger.info(
            f"New worker checked-in: {worker.name} by {worker.user.get_unique_alias()}"
        )
        self.initiate_save()
    ...
    def get_available_models(self, waiting_prompts, lite_dict=False):
        ...
        for worker in self.workers.values():
            ...
            for model_name in worker.models:
                ...
                models_dict[model_name] = models_dict.get(
                    model_name, mode_dict_template
                )
    ...
    def transfer_kudos_from_apikey_to_username(
        self, source_api_key, dest_username, amount
    ):
