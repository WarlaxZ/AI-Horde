...
class WaitingPrompt(WaitingPrompt):
    ...
    def activate(self):
        ...
        super().activate()
        logger.info(
            f"New prompt with ID {self.id} by {self.user.get_unique_alias()}: token:{self.max_length} * n:{self.n} == {self.total_usage} Total Tokens"
        )
...
class Worker(Worker):
    def check_in(self, max_length, max_content_length, softprompts, **kwargs):
        ...
        logger.debug(
            f"Worker {self.name} checked-in, offering models {self.models} at {self.max_length} max tokens and {self.max_content_length} max content length."
        )

    @logger.catch(reraise=True)
    def calculate_uptime_reward(self):
        return round(
            self.db.stats.calculate_model_multiplier(self.models[0]) * 25 / 2.75, 2
        )
...
class Stats(Stats):
    ...
    def calculate_model_multiplier(self, model_name):
        ...
        try:
            ...
            logger.error(
                f"Model '{model_name}' not found in hugging face. Defaulting to multiplier of 1."
            )
            multiplier = 1
