...
class WaitingPrompt(WaitingPrompt):
    ...
    def extract_params(self, params, **kwargs):
        ...
        if self.n > 20:
            logger.warning(
                f"User {self.user.get_unique_alias()} requested {self.n} gens per action. Reducing to 20..."
            )
            self.n = 20
    ...
    def activate(self):
        ...
        logger.info(
            f"New {prompt_type} prompt with ID {self.id} by {self.user.get_unique_alias()} ({self.ipaddr}): w:{self.width} * h:{self.height} * s:{self.steps} * n:{self.n} == {self.total_usage} Total MPs"
        )
    ...
    def calculate_kudos(self):
        result = pow((self.width * self.height) - (64 * 64), 1.75) / pow(
            (1024 * 1024) - (64 * 64), 1.75
        )
...
class Worker(Worker):
    def check_in(self, max_pixels, **kwargs):
        ...
        logger.debug(
            f"{paused_string}Worker {self.name} checked-in, offering models {self.models} at {self.max_pixels} max pixels"
        )
    ...
    def can_generate(self, waiting_prompt):
        ...
        if waiting_prompt.source_processing not in [
            "inpainting",
            "outpainting",
        ] and self.models == ["stable_diffusion_inpainting"]:
            is_matching = False
            skipped_reason = "models"
        ...
        if waiting_prompt.gen_payload.get("karras", False) and self.bridge_version < 6:
            ...
            skipped_reason = "bridge_version"
        if (
            len(waiting_prompt.gen_payload.get("post_processing", [])) >= 1
            and self.bridge_version < 7
        ):
            is_matching = False
            skipped_reason = "bridge_version"
